...
Taken from raine, I love the way this thing preserves comments in config files
...
typedef struct CONFIG
{
   ...
   char *data_buf;
   ui32 data_size;	// size allocated
   ui32 current_size; // size actually used so far
} CONFIG;
...
static bool config_installed = false;
...
char *get_data_ptr(CONFIG *cfg,ui32 size) {
  char *new_ptr;
...
int csb_get_config_int(const char *section, const char *name, int def)
{
   ...
   /* read normal data */
   s = csb_get_config_string(section, name, NULL);
...
{
   char *s = csb_get_config_string(section, name, NULL);
   ui32 i;
...
void csb_set_config_string(const char *section, const char *name, char *val)
{
   ...
   char section_name[256];
   if (!*name) {
       fatal_error("csb_set_config_string: no name !");
   }
...
void csb_set_config_int(const char *section, const char *name, int val)
{
   char buf[32];
   sprintf(buf, "%d", val);
   csb_set_config_string(section, name, buf);
}
...
void csb_set_config_hex(char *section, char *name, int val)
{
   if (val >= 0) {
      ...
      csb_set_config_string(section, name, "-1");
}
